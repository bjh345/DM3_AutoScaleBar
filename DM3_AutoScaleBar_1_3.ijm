...
function calculate(UL, UR, LL, LR, size) {
	...
	Array.getStatistics(array1, min);
	if (min >= 60000){colour = "Black";}
	else if (min <= 59999){colour = "White";}
	if (min_val[0] == 0){
	...
	addScaleBar(size, placement, colour);
}

function addScaleBar(size, placement, colour) {

	if (size >1) {
		run("Scale Bar...", "width=size height=40 font=90 color=[&colour] background=None location=[&placement] bold");
	}
	else {}
}