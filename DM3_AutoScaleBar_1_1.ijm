...
function processFolder(input, outputT, outputJ) {
	...
	for (i = 0; i < list.length; i++) {
		if(File.isDirectory(input + list[i]))
			...
			//print("subfolders ignored");
			//gen_folders(""+ outputT + File.separator + list[i], ""+ outputJ + File.separator + list[i]);
			//processFolder("" + input + File.separator +  list[i], ""+ outputT + File.separator + list[i], ""+ outputJ + File.separator + list[i]);
